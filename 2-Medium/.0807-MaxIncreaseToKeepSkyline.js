...
function MaxIncreaseToSkyline(arr) {
    let maxNums = [
        [],
        []
    ];
    ...
    for( let i=0; i<arr.length; i++ ) {
        for( let j=0; j<arr[i].length; j++ ) {
            if( arr[i][j] > tempMaxRow ) {
                tempMaxRow = arr[i][j];
            };
            if( arr[j][i] > tempMaxCol ) {
                tempMaxCol = arr[j][i];
            };
        };
        maxNums[0].push(tempMaxRow);
        tempMaxRow = 0;
        maxNums[1].push(tempMaxCol);
        tempMaxCol = 0;
    };
    console.log(maxNums);
    ...
    for( let i=0; i<arr.length; i++ ) {
        for( let j=0; j<arr[i].length; j++ ) {
            if( maxNums[0][j] > arr[j][i] ) {
                count += ( maxNums[0][j] - arr[j][i] );
            } else if( maxNums[0][j] < arr[j][i] ) {
                count -= ( arr[j][i] - maxNums[0][j] )
            };
            if( maxNums[1][j] > arr[i][j] ) {
                count += ( maxNums[1][j] - arr[i][j] );
            } else if( maxNums[1][j] < arr[i][j] ) {
                count -= ( arr[i][j] - maxNums[1][j] );
            };
        };
    };
...
console.log( MaxIncreaseToSkyline(arr1) );