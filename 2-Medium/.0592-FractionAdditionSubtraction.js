...
// If your final result is an integer, say 2, you need to change it 
// to the format of fraction that has denominator 1. 
...
function fractionalAdditionSubtraction( string ) {
    ...
    let subFraction = "";
    let isStart = true;
    while( i < stringCopy.length ) {
        if( isStart ) {
            subFraction += stringCopy[i]
            isStart = false
            
            i++;
        } else {
            if( stringCopy[i] === '+' || stringCopy[i] === '-' ) {
                fractionArray.push( subFraction )
                subFraction = "";
                stringCopy = stringCopy.substring( i );
                isStart = true;
...
const str1 = "-1/2+1/2";
const str2 = "-1/2+1/2+1/3";
const str3 = "1/3-1/2";
const str4 = "5/3+1/3";

fractionalAdditionSubtraction( str1 )